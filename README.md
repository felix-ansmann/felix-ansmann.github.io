...
| `images` | optional | Array of image paths shown as slideshow on hover — see section below |
...
## How to Add Hover Images (Slideshow)

When someone hovers over a project row on desktop, the images cycle through one by one — each shown for 1 second, no transitions.

**Step 1 — Create a folder for the project**

Inside `assets/projects/`, create a subfolder with a short name (no spaces):
```
assets/projects/extreme-value/
assets/projects/closed-loop/
```

**Step 2 — Drop your images in**
- Supported formats: JPG, PNG, WebP
- Recommended width: 1200–1800 px
- Keep each file under 400 KB
- Name them simply: `01.jpg`, `02.jpg`, `03.jpg` — or any name without spaces

**Step 3 — List them in projects.js**

Add an `images` field to the project entry:
```js
images: [
...
The images cycle in the order listed. If only one image is in the list, it just shows that image without cycling.

If the field is missing or all paths are wrong, no hover panel appears — nothing breaks.
